...
import { scoreBatchWithKeywords } from './fallback-scorer';
...
[
  ...
  ...
    ...
    try {
      ...
      if (!jsonMatch) {
        console.log('[ML Service] Failed to parse Claude response, using keyword-based fallback');

        // Fallback: Use keyword-based scoring
        const fallbackScores = scoreBatchWithKeywords(batch, approvedExamples, junkExamples);
      ...
      console.log('[ML Service] Claude API failed, using keyword-based fallback scorer');

      // Fallback: Use keyword-based scoring (reinforcement learning from training data)
      const fallbackScores = scoreBatchWithKeywords(batch, approvedExamples, junkExamples);
      const scoredBatch = batch.map((article, index) => {
        const score = fallbackScores[index];
        const isJunk = junkLinks.has(article.link);
