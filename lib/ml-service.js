...
[
  ...
  ...
    ...
    try {
      ...
      if (!jsonMatch) {
        console.log('[ML Service] Failed to parse Claude response, using neural network fallback');

        // Fallback: Use neural network with backpropagation
        const fallbackScores = scoreBatchWithNeuralNet(batch, approvedExamples, junkExamples);
      ...
      console.log('[ML Service] Claude API failed, using neural network fallback scorer');
