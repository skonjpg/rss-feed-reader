...
export async function scoreBatchArticles(articles) {
  ...
  // Build articles to score
  const articlesText = articles
    .slice(0, 10) // Limit to 10 articles per batch to avoid token limits
  ...
  try {
    ...
    if (!jsonMatch) {
      ...
      return await Promise.all(
        articles.slice(0, 10).map(article => scoreArticleWithClaude(article, approvedExamples))
      );
    }
