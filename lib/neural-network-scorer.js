...
// Cache for trained model to avoid retraining every time
...
export function scoreBatchWithNeuralNet(articles, approvedArticles, junkArticles) {
  try {
    ...
    if (cacheValid) {
      console.log('[Neural Network] ⚡ Using cached model (instant scoring)');
      nn = cachedModel.nn;
      vocabulary = cachedModel.vocabulary;
    } else {
      console.log('[Neural Network] Training new model (this will take 10-30 seconds)...');
      ...
      // Cache the model
      cachedModel = { nn, vocabulary };
      cacheTimestamp = now;
      console.log('[Neural Network] ✅ Model trained and cached for 5 minutes');
    }
...
/**
 * Invalidate the model cache (call when new articles are approved/junked)
 */
export function invalidateModelCache() {
  cachedModel = null;
  cacheTimestamp = null;
  console.log('[Neural Network] Model cache invalidated');
}
