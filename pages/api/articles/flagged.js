...
export default async function handler(req, res) {
  ...
  try {
    // Fetch all flagged articles, ordered by flagged date (newest first)
    ...
    if (error) {
      ...
      return res.status(500).json({ error: 'Failed to fetch flagged articles', details: error.message });
    }
